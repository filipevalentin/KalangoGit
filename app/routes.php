...
Route::group(array('prefix' => 'admin', 'before'=>'admin'), function(){
	...
	//Modulos
		...
		Route::post('criarModulo', function(){
			...
			if($curso->modulos()->count() != null){
				$modulos = array();
				foreach ($curso->modulos()->get()->lists('nome') as $m) {
	...
	//Turmas
		...
		Route::post('criarTurma', function(){
			...
			if($curso->turmas()->count() != null){
				$turmas = array();
				foreach ($curso->turmas()->get()->lists('nome') as $m) {
					$turmas[] = strtolower($m);
				}
	...
	//Aulas
		...
		Route::post('criarAula', function(){
			...
			$modulo = Modulo::find($Aula->idModulo);

			if($modulo->aulas()->count() != null){
				$aulas = array();
				foreach ($modulo->aulas()->get()->lists('titulo') as $m) {
					$aulas[] = strtolower($m);
				}

				if(in_array(strtolower(Input::get('nome')), $aulas) ){
	...
	//Materiais
		...
		Route::post('criarMaterial', function(){
			...
			if($aula->materialApoio()->count() != null){
				$materiais = array();
				foreach ($aula->materialApoio()->get()->lists('nome') as $m) {
